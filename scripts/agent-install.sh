...
command -v systemctl >/dev/null || die "v0.1 Agent 需要 systemd"
...
if command -v apt-get >/dev/null; then
  ...
  die "当前安装器支持 Debian/Ubuntu/RHEL 系"
fi
...
info "向控制面注册"
enroll_json="$(TOKEN_VALUE="$TOKEN" node -e 'process.stdout.write(JSON.stringify({token:process.env.TOKEN_VALUE,hostname:require("node:os").hostname(),version:"0.1.0",system:{platform:process.platform,arch:process.arch}}))')"
...
systemctl enable --now nexusgate-xray.service nexusgate-agent.service
printf '\n\033[1;32mAgent 安装并注册完成。\033[0m\n'
