...
class Orchestrator {
  ...
  async deployChain(chainId, actor = 'admin') {
    return this.store.transaction((data) => {
      ...
      if (['vless-ws-tls', 'hysteria2'].includes(chain.relayProtocol)) {
  ...
  async recordUsage(serverId, samples, error = null, epoch = null) {
    return this.store.transaction((data) => {
      ...
      for (const sample of Array.isArray(samples) ? samples.slice(0, 1000) : []) {
        ...
        const restarted = Boolean(epoch && previous.epoch && epoch !== previous.epoch);
        ...
        deployment.usageCounters = { uplink, downlink, epoch: epoch || null };
