...
class Orchestrator {
  ...
  async deployChain(chainId, actor = 'admin') {
    return this.store.transaction((data) => {
      ...
      if (chain.relayProtocol === 'vless-ws-tls') {
        const missing = entryServers.filter((server) => !server.tlsDomain);
        if (missing.length) { const error = new Error(`VLESS WS TLS 需要先填写设备 TLS 域名并在节点上安装证书：${missing.map((server) => server.name).join('、')}`); error.statusCode = 400; throw error; }
      }
      ...
      const unavailable = [...new Map([...entryServers, ...(exitServer ? [exitServer] : [])].map((server) => [server.id, server])).values()]
        .filter((server) => server.status !== 'online' || Date.now() - Date.parse(server.lastSeenAt || 0) > 120000 || server.engine?.status === 'error');
      if (unavailable.length) {
        const detail = unavailable.map((server) => `${server.name}（${server.engine?.status === 'error' ? 'Xray 故障：' + server.engine.detail : 'Agent 离线；请在该机器运行 ng-agent doctor'}）`).join('、');
      ...
      if (success && job.action === 'apply_resource' && (deployment.archived || !chain)) {
        ...
        if (deployment.role === 'relay' || deployment.role === 'direct') {
          ...
          deployment.clientUri = buildClientUri({
            ...
            name: `${chain.name} · ${server ? server.name : '入口'}:${deployment.port}`,
