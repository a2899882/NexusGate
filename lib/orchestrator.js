  ...
  async deployChain(chainId, actor = 'admin') {
    return this.store.transaction((data) => {
      ...
      if (chain.relayProtocol === 'anytls') {
        const missing = entryServers.filter((server) => {
          ...
          return !agent || !(parts[0] > 0 || (parts[0] === 0 && (parts[1] > 6 || (parts[1] === 6 && parts[2] >= 1)))) || !server.engine?.singBoxInstalled;
        });
        if (missing.length) {
          const error = new Error(`AnyTLS 入口需要新版 Agent 和 sing-box：请在 ${missing.map((server) => server.name).join('、')} 运行 ng-agent update，再执行 ng-agent engine install`);
